...
import {Medication} from "@/pages/dashboard/chicken/manage/medication/add.tsx";
...
export default function MedicationList() {
    ...
    const {data: movements, isLoading, isFetching, refetch} = useQuery({
        queryKey: ['medicationLogs', params.chickenId],
        queryFn: () =>
            pb.collection('medicationLogs').getFullList<Medication>({
                sort: '-date',
    ...
    const handleDelete = (id: string) => {
        alertDialog.open({
            ...
            if (res) {
                pb.collection('medicationLogs').delete(id).then(() => {
                    refetch();
                })
            }
        })
        ...
        <div>
            <div className={'flex items-center justify-between gap-2'}>
                <div className={'flex items-center gap-2'}>
                    <h1 className={'font-bold text-xl line-clamp-1 w-full'}>Medication Logs</h1>
                </div>
                <div>
                    <Link className={buttonVariants({size: 'sm', variant: 'default'})}
                          to={`/dashboard/chickens/${params.chickenId}/manage/medication/+`}>
...
function MedicationCard({log, handleDelete}: {
    log: Medication,
        ...
        <div
            ...
            <div className={'flex gap-2'}>
                <Link className={buttonVariants({size: 'icon', variant: 'ghost'})}
                      to={`/dashboard/chickens/${log.chickenBatchId}/manage/medication/${log.id}`}>
