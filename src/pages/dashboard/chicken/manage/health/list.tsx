...
import {HealthLog} from "@/pages/dashboard/chicken/manage/health/add.tsx";
...
export default function HealthList() {
    ...
    const {data: healthLogs, isLoading, isFetching, refetch} = useQuery({
        queryKey: ['healthLogs', params.chickenId],
        queryFn: () =>
            pb.collection('healthLogs').getFullList<HealthLog>({
                sort: '-date',
    ...
    const handleDelete = (id: string) => {
        alertDialog.open({
            ...
            if (res) {
                pb.collection('healthLogs').delete(id).then(() => {
                    refetch();
                })
            }
        })
        ...
        <div>
            <div className={'flex items-center justify-between gap-2'}>
                <div className={'flex items-center gap-2'}>
                    <h1 className={'font-bold text-xl line-clamp-1 w-full'}>Health Logs</h1>
                </div>
                <div>
                    <Link className={buttonVariants({size: 'sm', variant: 'default'})}
                          to={`/dashboard/chickens/${params.chickenId}/manage/health/+`}>
            ...
            <div className={'flex flex-col gap-2 py-4'}>
                {
                    ...
                    isLoading || isFetching ?
                        <div className={'space-y-2'}>
                            <HealthLogCardSkeleton/>
                            <HealthLogCardSkeleton/>
                            <HealthLogCardSkeleton/>
                        </div>
                        : healthLogs?.map((log) =>
                            <HealthLogCard key={log.id}
                                           handleDelete={handleDelete}
                                           log={log}/>)
                }
                {isLoading || isFetching ? null : healthLogs?.length ? null :
                    <div className={'flex justify-center items-center flex-col gap-2 py-20'}>
                        <p className={'text-gray-500 text-sm'}>No health logs found</p>
                        <Link className={buttonVariants({size: 'sm', variant: 'default'})}
                              to={`/dashboard/chickens/${params.chickenId}/manage/health/+`}>
                            <span>Add Health Log</span>
                        </Link>
        ...
        <div
            ...
            <div>
                <p className={'text-lg font-bold'}>
                    <span>{format(new Date(log.date), 'dd MMM, yyyy')} - {log.status}</span>
                </p>
            </div>
            <div className={'flex gap-2'}>
                <Link className={buttonVariants({size: 'icon', variant: 'ghost'})}
                      to={`/dashboard/chickens/${log.chickenBatchId}/manage/health/${log.id}`}>
